import { defineComponent, PropType, ref } from 'vue';
import s from './ItemCreate.module.scss'
import { MainLayout } from '../../layouts/MainLayout';
import { Icon } from '../../shared/Icon';
import { Tabs, Tab } from '../../shared/Tabs';
import { InputPad } from './InputPad';
export const ItemCreate = defineComponent({
  props: {
    name: {
      type: String as PropType<string>
    }
  },
  setup: (props, context) => {
    const moneyKind = ref('支出')
    const expensesTags = ref([
      { id: 1, name: '餐饮', sign: '￥', category: 'expenses' },
      { id: 2, name: '购物', sign: '￥', category: 'expenses' },
      { id: 3, name: '日用', sign: '￥', category: 'expenses' },
      { id: 4, name: '交通', sign: '￥', category: 'expenses' },
      { id: 5, name: '蔬菜', sign: '￥', category: 'expenses' },
      { id: 6, name: '水果', sign: '￥', category: 'expenses' },
      { id: 7, name: '运动', sign: '￥', category: 'expenses' },
    ])
    const incomeTags = ref([
      { id: 1, name: '工资', sign: '￥', category: 'income' },
      { id: 2, name: '兼职', sign: '￥', category: 'income' },
      { id: 3, name: '理财', sign: '￥', category: 'income' },
      { id: 4, name: '礼金', sign: '￥', category: 'income' },
      { id: 5, name: '其他', sign: '￥', category: 'income' },
    ])
    return () => (
      <MainLayout class={s.layout}>{{
        title: () => '记账',
        icon: () => <Icon class={s.navIcon} name="cancel" />  ,
        default: () => <>
          <div class={s.wrapper}>
            <Tabs v-model:selected={moneyKind.value} class={s.tabs}>
              <Tab name='支出' class={s.tags_wrapper}>
                <div class={s.tag}>
                  <div class={s.sign}>
                    <Icon name='plus' class={s.createTag} />
                  </div>
                  <div class={s.name}>新增</div>
                </div>
                {expensesTags.value.map(tag => 
                  <div class={[s.tag, s.selected]}>
                    <div class={s.sign}>
                      {tag.sign}
                    </div>
                    <div class={s.name}>
                      {tag.name}
                    </div>
                  </div>  
                )}
              </Tab>
              <Tab name="收入" class={s.tags_wrapper}>
                <div class={s.tag}>
                  <div class={s.sign}>
                    <Icon name="plus" class={s.createTag} />
                  </div>
                  <div class={s.name}>
                    新增
                  </div>
                </div>
                {incomeTags.value.map(tag =>
                  <div class={[s.tag, s.selected]}>
                    <div class={s.sign}>
                      {tag.sign}
                    </div>
                    <div class={s.name}>
                      {tag.name}
                    </div>
                  </div>
                )}
              </Tab>
            </Tabs>
            <div class={s.inputPad_wrapper}>
              <InputPad />
            </div>
          </div>
        </>
      }}</MainLayout> 
    )
  }
})